...
function AdminDashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const [usersRes, booksRes, reviewsRes, viewsRes] = await Promise.all([supabase.from('profiles').select('id', {
        count: 'exact',
        head: true
        ...
        count: 'exact',
        head: true
      }), supabase.from('book_views').select('id', {
        count: 'exact',
        head: true
      })]);
      return {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('profiles').select('subscription_tier');
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('book_views').select(`
          *,
          books (title, author),
        ...
        <Card className="mb-8 bg-gradient-primary text-white">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 bg-[#f2dfe9]">
                <Shield className="h-8 w-8" />
                <div>
                  <CardTitle className="text-3xl text-[#390620]">Admin Dashboard</CardTitle>
                  <CardDescription className="text-[#33091e]">
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Button variant="default" className="w-full justify-start h-auto py-4" onClick={() => navigate('/admin/import')}>
        ...
        <Tabs defaultValue="activity" className="w-full">
          ...
          <TabsContent value="activity" className="mt-6">
            <Card>
              ...
              <CardContent>
                {recentActivity && recentActivity.length > 0 ? <div className="space-y-4">
                    {recentActivity.map((activity: any) => <div key={activity.id} className="flex items-center justify-between border-b pb-3 last:border-0">
                  ...
                  </div> : <p className="text-center text-muted-foreground py-8">No recent activity</p>}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="reviews" className="mt-6">
            <Card>
              ...
              <CardContent>
                {recentReviews && recentReviews.length > 0 ? <div className="space-y-4">
                    {recentReviews.map((review: any) => <div key={review.id} className="border-b pb-4 last:border-0">
                        <div className="flex items-start gap-3 mb-2">
                          {review.books?.cover_url && <img src={review.books.cover_url} alt={review.books.title} className="w-12 h-16 object-cover rounded" />}
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {review.pen_name && <Badge variant="outline" className="text-xs">{review.pen_name}</Badge>}
                            </div>
                          </div>
                  ...
                  </div> : <p className="text-center text-muted-foreground py-8">No reviews yet</p>}
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
...
function UserProfile() {
  ...
  return <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <CardHeader>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                {profile?.avatar_url && <img src={profile.avatar_url} alt={profile.full_name || 'User'} className="w-20 h-20 rounded-full object-cover" />}
                <div>
        ...
        {profile?.subscription_tier === 'free' && freeViewsRemaining !== null && <Card className="mb-8">
            <CardHeader>
              ...
              <CardDescription>
                ...
                {profile.free_views_reset_date && <> Resets on {format(new Date(profile.free_views_reset_date), 'MMM dd, yyyy')}.</>}
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        {profile?.subscription_tier === 'premium_monthly' && <Card className="mb-8">
            <CardHeader>
              <CardTitle>Premium Monthly Subscription</CardTitle>
              <CardDescription>
                Status: {profile.subscription_status}
                {profile.subscription_ends_at && <> • Renews on {format(new Date(profile.subscription_ends_at), 'MMM dd, yyyy')}</>}
              </CardDescription>
            </CardHeader>
          </Card>}
        ...
        <Tabs defaultValue="reviews" className="w-full">
          ...
          <TabsContent value="reviews" className="mt-6">
            {reviewsLoading ? <div className="flex justify-center py-8">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
              </div> : reviews && reviews.length > 0 ? <div className="grid gap-4">
                {reviews.map((review: any) => <Card key={review.id} className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate(`/book/${review.book_id}`)}>
                    <CardHeader>
                      <div className="flex items-start gap-4">
                        {review.books?.cover_url && <img src={review.books.cover_url} alt={review.books.title} className="w-16 h-24 object-cover rounded" />}
                        <div className="flex-1">
                          <CardTitle className="text-lg">{review.books?.title}</CardTitle>
                          <CardDescription>{review.books?.author}</CardDescription>
                          <div className="flex items-center gap-2 mt-2">
                            <div className="text-red-500">{'❤️'.repeat(review.hearts || 0)}</div>
                            {review.pen_name && <Badge variant="outline">{review.pen_name}</Badge>}
                          </div>
                        </div>
                      </div>
                    </CardHeader>
          ...
          <TabsContent value="books" className="mt-6">
            {viewsLoading ? <div className="flex justify-center py-8">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
              </div> : bookViews && bookViews.length > 0 ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {bookViews.map((view: any) => <Card key={view.id} className="cursor-pointer hover:shadow-lg transition-shadow overflow-hidden" onClick={() => navigate(`/book/${view.book_id}`)}>
                    {view.books?.cover_url ? <img src={view.books.cover_url} alt={view.books.title} className="w-full h-48 object-cover" /> : <div className="w-full h-48 bg-muted flex items-center justify-center">
